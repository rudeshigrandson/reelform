...
import { createRemuxPostProcess, remuxedPath } from "./remuxPostProcess";
...
describe("createRemuxPostProcess", () => {
  it("remuxes every WebM track to MP4 with stream copy, then drops the WebM", async () => {
    const { spawn, calls } = scriptedSpawn(({ child }) => child.close(0));
    const fs = fakeFs({ "/rec/r1/screen.mp4": 900, "/rec/r1/mic.mp4": 150 });
    ...
    expect(out.mic).toEqual({ path: "/rec/r1/mic.mp4", bytes: 150 });
  ...
  it("keeps WebM and removes the partial MP4 when ffmpeg fails for a track", async () => {
    let n = 0;
    ...
    const fs = fakeFs({ "/rec/r1/mic.mp4": 150 });
    const log = vi.fn();
