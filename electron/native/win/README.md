...
- In: `ping`, `start`, `pause`, `resume`, `stop`, `discard`. `start` accepts `outputDir` or `outDir` (the key `electron/capture/helperBackend.ts` sends) and `source.id` in place of `displayId`/`windowId`. Windows extensions on `start`: `source.bounds` (virtual-desktop **physical** px; picks the HMONITOR by largest overlap, since Electron display ids do not map to HMONITORs — main should send `screen.dipToScreenRect(null, display.bounds)`; DIP bounds usually still resolve, see `capture_resolve_monitor_dip_bounds`), `hideCursor`, and the microphone mapping fields `audio.micEndpointId` (exact `IMMDevice::GetId`) and `audio.micLabel` (`MediaDeviceInfo.label`, matched against `PKEY_Device_FriendlyName`; see `common/include/reelform/audio_devices.hpp`).
- Out: `ready` (once, at launch), `pong{version,caps}` (caps include `capture`), `ready{id}` (reply to a successful `start`), `started{firstFramePtsNs,startHostTimeNs,width,height,scaleFactor}` (first frame, echoes the start `id`), `paused{id}` / `resumed{id}` acks, `stats{fps,droppedFrames,fileBytes,micLevel?}` every 1 s, `interrupted{reason,message}`, `deviceLost{device,message}`, `stopped{durationMs,paths{screen,system?,mic?,meta?},pausedRanges,discarded}`, `error{code,message,fatal}`.
- `interrupted.reason`: `sourceLost` (window closed, display gone), `streamStopped` (device removed, duplication failed), `writerFailed` (encoder failure), `diskLow` (disk < 500 MB), `parentGone` (stdin closed). `deviceLost` stays in the shared set but Windows no longer emits it as an interruption.
- Microphone loss is non-fatal (as on macOS): the helper sends `deviceLost{device,message}` and keeps recording video and system audio.
...
- A successful `start` is answered by `ready{id}` immediately and `started` on the first frame. A failed start gets `error` with the start `id`. `pause` and `resume` answer `paused{id}` / `resumed{id}`, so main's request correlation resolves.
...
- **Mic device ids.** Chromium's `getUserMedia` deviceIds are hashed and never match an endpoint. Resolution order: `audio.micEndpointId`, then `audio.mic` as an endpoint id, then `audio.micLabel` against endpoint friendly names (exact after stripping Chromium's `Default - ` / `Communications - ` prefix, else a unique substring match), else the default capture device with `error{code:"micUnavailable",fatal:false}`. Main must send `micLabel` (or an endpoint id) for a non-default mic to work.
- **System audio device change.** A loopback endpoint change mid-recording (for example, headphones plugged in) emits `error{code:"streamFailed",fatal:false}`. Recording continues and the system track is padded with silence. Losing the microphone sends `deviceLost` and recording continues, as on macOS.
...
- **No `listSources`.** The Windows helpers answer it with `unknownCommand`; source listing on Windows comes from Electron's `desktopCapturer`.
- **Encoder failure mid-recording.** If the hardware encoder fails after start, the helper stops with `interrupted{reason:"writerFailed"}` and keeps the fragmented MP4. There is no live switch to the software encoder.
