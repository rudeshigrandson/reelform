...
import { summarizeShortcuts, type KeystrokeCandidate } from "./keystrokes";
import {
  ANNOTATION_KINDS,
  TEXT_FONTS,
  TOOL_GLYPHS,
...
function NoSelection({
  ...
  return (
    <>
      ...
      {showKeystrokes && <KeystrokeList shortcuts={detectedShortcuts} onAddAll={onAddAllShortcuts} />}
      <EmptyState title="No annotation selected">
        <ul style={{ margin: 0, paddingLeft: "var(--space-4)", display: "flex", flexDirection: "column", gap: "2px" }}>
...
function KeystrokeList({
  ...
  return (
    <Section title="Keystroke badges">
      {n === 0 ? (
        <EmptyState title="No shortcuts detected">Shortcuts pressed while recording will appear here.</EmptyState>
      ) : (
        <>
...
function SelectedPanels(props: AnnotationsInspectorProps & { selected: Annotation }): ReactElement {
  const { selected: a, onChange, onDuplicate, onDelete, timelineDurationMs = Infinity } = props;
  return (
    <>
      <div style={{ ...rowStyle, justifyContent: "space-between" }}>
      ...
      <Section title="Animation">
        <AnimControls label="In" name={`${a.id}-anim-in`} anim={a.animIn} onChange={(animIn) => onChange({ ...a, animIn })} />
        <AnimControls
          label="Out"
          name={`${a.id}-anim-out`}
          anim={a.animOut}
      ...
      <Section title="Position">
        <NumberField label="X" unit="%" step={0.5} min={0} max={100} value={pct(a.x)} onChange={(v) => onChange({ ...a, x: v / 100 })} />
        <NumberField label="Y" unit="%" step={0.5} min={0} max={100} value={pct(a.y)} onChange={(v) => onChange({ ...a, y: v / 100 })} />
        <NumberField label="W" unit="%" step={0.5} min={0.5} max={100} value={pct(a.w)} onChange={(v) => onChange({ ...a, w: v / 100 })} />
        <NumberField label="H" unit="%" step={0.5} min={0.5} max={100} value={pct(a.h)} onChange={(v) => onChange({ ...a, h: v / 100 })} />
        <NumberField
          label="Rotation"
          unit="°"
          min={-180}
...
function AnimControls({
  ...
  return (
    <div role="group" aria-label={`Animation ${label.toLowerCase()}`} style={{ display: "flex", flexDirection: "column", gap: "var(--space-1)" }}>
      <Row label={label}>
        <Segmented name={name} value={anim.type} options={ANIM_OPTIONS} onChange={(type) => onChange({ ...anim, type })} />
      </Row>
      {anim.type === "slide" && (
        <Row label="From">
          <Segmented
...
function KindPanel({ a, onChange }: { a: Annotation; onChange: (a: Annotation) => void }): ReactElement | null {
  switch (a.kind) {
    case "text":
    ...
    case "line":
      return (
        <Section title="Stroke">
          <Slider label="Width" unit="px" min={1} max={32} value={a.strokeWidth} onChange={(strokeWidth) => onChange({ ...a, strokeWidth })} />
          ...
          <Switch label="Dashed" checked={a.dashed} onChange={(dashed) => onChange({ ...a, dashed })} />
        </Section>
      );
    case "rect":
    case "ellipse":
      return (
        <Section title="Shape">
          <ColorField label="Fill" value={a.fill.slice(0, 7)} onChange={(fill) => onChange({ ...a, fill })} />
          <ColorField label="Stroke" value={a.stroke.slice(0, 7)} onChange={(stroke) => onChange({ ...a, stroke })} />
          <Slider label="Stroke width" unit="px" min={0} max={32} value={a.strokeWidth} onChange={(strokeWidth) => onChange({ ...a, strokeWidth })} />
          <OpacitySlider a={a} onChange={onChange} />
          {a.kind === "rect" && (
            <Slider label="Radius" unit="px" min={0} max={64} value={a.radius} onChange={(radius) => onChange({ ...a, radius })} />
          )}
        </Section>
      );
    case "highlight":
      return (
        <Section title="Highlight">
          <ColorField label="Color" value={a.fill.slice(0, 7)} onChange={(fill) => onChange({ ...a, fill })} />
          <OpacitySlider a={a} onChange={onChange} />
          <Slider label="Radius" unit="px" min={0} max={64} value={a.radius} onChange={(radius) => onChange({ ...a, radius })} />
        </Section>
      );
    case "blur":
      return (
        <Section title="Blur">
          <Slider label="Strength" min={1} max={64} value={a.strength} onChange={(strength) => onChange({ ...a, strength })} />
          <Switch label="Pixelate" hint="Mosaic instead of blur" checked={a.pixelate} onChange={(pixelate) => onChange({ ...a, pixelate })} />
        </Section>
      );
    case "image":
      return (
        <Section title="Image">
          ...
          <OpacitySlider a={a} onChange={onChange} />
          <Row label="Fit">
            <Segmented name={`${a.id}-fit`} value={a.fit} options={FIT_OPTIONS} onChange={(fit) => onChange({ ...a, fit })} />
          </Row>
          <Slider
            label="Corner radius"
            unit="px"
    ...
    case "emoji":
      return (
        <Section title="Emoji">
          <Input label="Emoji" value={a.emoji} maxLength={8} onChange={(e) => onChange({ ...a, emoji: e.target.value })} />
        </Section>
      );
    case "numberBadge":
      return (
        <Section title="Badge">
          <NumberField label="Number" min={0} max={999} value={a.value} onChange={(value) => onChange({ ...a, value: Math.round(value) })} />
          <ColorField label="Fill" value={a.fill} onChange={(fill) => onChange({ ...a, fill })} />
          <ColorField label="Text" value={a.color} onChange={(color) => onChange({ ...a, color })} />
        </Section>
      );
    case "keystrokeBadge":
      return (
        <Section title="Keystroke">
          <Input label="Label" value={a.label} onChange={(e) => onChange({ ...a, label: e.target.value })} />
        </Section>
      );
  }
}

function OpacitySlider({ a, onChange }: { a: Annotation; onChange: (a: Annotation) => void }): ReactElement {
  return (
    <Slider
      label="Opacity"
      unit="%"
...
function TextPanel({ a, onChange }: { a: AnnotationOf<"text">; onChange: (a: Annotation) => void }): ReactElement {
  return (
    <Section title="Text">
      <Textarea label="Content" rows={3} value={a.text} onChange={(e) => onChange({ ...a, text: e.target.value })} />
      <Row label="Font">
        <select
          aria-label="Font"
          className="input"
      ...
      <Slider label="Size" unit="px" min={8} max={160} value={a.fontSize} onChange={(fontSize) => onChange({ ...a, fontSize })} />
      <Row label="Weight">
        <Segmented
          name={`${a.id}-weight`}
          value={a.fontWeight}
      ...
      <ColorField label="Color" value={a.color} onChange={(color) => onChange({ ...a, color })} />
      <Switch label="Background pill" checked={a.background} onChange={(background) => onChange({ ...a, background })} />
      {a.background && (
        <ColorField
          label="Pill color"
      ...
      <Slider label="Padding" unit="px" min={0} max={48} value={a.padding} onChange={(padding) => onChange({ ...a, padding })} />
      <Row label="Align">
        <Segmented name={`${a.id}-align`} value={a.align} options={ALIGN_OPTIONS} onChange={(align) => onChange({ ...a, align })} />
      </Row>
    </Section>
  );
}
